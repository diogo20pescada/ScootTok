...
const uploadStorage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    ...
    const safeExt = originalExt ? originalExt.toLowerCase() : ".mp4"
...
app.post("/upload", upload.single("video"), (req, res) => {
  ...
  const id = Date.now()
  db.prepare(`
    INSERT INTO videos (id, user_username, title, description, file_name, mimetype, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(id, user, title, desc, req.file.filename, String(req.file.mimetype || ""), id)
